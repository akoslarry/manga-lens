...
- **修复纯场景画面 PDF 缺页**：当漫画中包含纯画面（无文字气泡）的图片页时，OCR 不会返回任何文字，此前直接跳过导致 PDF 导出复选框无法勾选该图片，最终导出 PDF 缺页。现在纯场景页也会注册容器并出现在 PDF 选择列表中。
- **缓存命中空对话修复**：从本地缓存加载纯场景页时（dialogs 为空），现在只创建容器不渲染覆盖层，同时正确注册到 PDF 复选框。
- **透明度滑块升级**：覆盖层透明度控制从数字输入框改为拖拽式滑块，支持点击轨道跳转和拖拽调节，操作更直观流畅。
